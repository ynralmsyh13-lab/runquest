...
<div style="font-family:'Exo 2',sans-serif;">
  ...
  <div id="sum" style="display:none; background:{c['CARD']}; border:1.5px solid {c['BORDER2']}; border-radius:8px; padding:16px; position:relative; overflow:hidden;">
    ...
    <div id="sum-status" style="text-align:center;font-family:Share Tech Mono,monospace;font-size:0.7rem;color:{c['TEXT3']};margin-top:8px;letter-spacing:1px;"></div>
    <div id="btn-save-confirm" style="display:none;margin-top:8px;">
        <div style="background:rgba(255,179,0,0.15);border:2px solid {c['ACCENT']};border-radius:6px;padding:12px;text-align:center;font-family:Share Tech Mono,monospace;font-size:0.75rem;color:{c['ACCENT']};letter-spacing:1px;">
            ✓ RUN DATA READY — USE THE SAVE BUTTON BELOW THE MAP
        </div>
    </div>
  </div>
</div>
...
function gst(){{
    ...
    setStatus('<span style="color:{c["TEXT3"]};font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ SESSION ENDED — SAVING... ]</span>');
    ...
    // Auto-save: write to Streamlit bridge input
    if(tD > 0.001 && tS > 0){{
        var bridgeVal = tD.toFixed(4) + '|' + tS + '|' + unit;
        // Find the hidden bridge text input in parent frame and set its value
        setTimeout(function(){{
            try {{
                ...
                for(var i=0; i<inputs.length; i++){{
                    ...
                    if(inp.value === '' && inp.placeholder === ''){{
                        ...
                        nativeInputValueSetter.call(inp, bridgeVal);
                        inp.dispatchEvent(new Event('input', {{bubbles: true}}));
                        inp.dispatchEvent(new Event('change', {{bubbles: true}}));
                        setStatus('<span style="color:{c["ACCENT"]};font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ SAVING RUN... PLEASE WAIT ]</span>');
                        break;
                    }}
                }}
            }} catch(e) {{
                setStatus('<span style="color:#FF8C00;font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ TAP CONFIRM SAVE BELOW ]</span>');
            }}
        }}, 500);
    }}
}}
...
function gsave(){{
    ...
    if(t<1){{alert('No run data! Press INITIATE first.');return;}}
    if(d<=0){{
        var dv=document.getElementById('dv');
        if(dv){{d=parseFloat(dv.textContent)||0;}}
    }}
    if(d<=0){{alert('Distance is 0. Run outside first!');return;}}
    // Write to hidden Streamlit text inputs via DOM
    var inputs = window.parent.document.querySelectorAll('input[type="text"]');
    ...
    document.getElementById('btn-save-confirm').style.display='block';
    document.getElementById('sum-status').textContent='Data ready! Press CONFIRM SAVE below.';
}}
</script>
...
def page_gps_tracker(user_id, profile, runs, unit, c):
    ...
    # Hidden bridge input — GPS JS writes "dist|dur|unit" here on HALT
    bridge_val = st.text_input("gps_bridge", value="", key="gps_bridge",
                                label_visibility="collapsed")

    # If bridge has data, auto-save immediately
    if bridge_val and "|" in bridge_val:
        try:
            parts = bridge_val.split("|")
            dist_raw = float(parts[0])
            dur_raw  = int(parts[1])
            u_raw    = parts[2] if len(parts) > 2 else unit_sel
            dist_km  = dist_raw if u_raw == "km" else dist_raw / 0.621371

            if dist_km > 0.001 and dur_raw > 0:
                total_xp, is_pb, fresh = process_and_save_run(
                    user_id, profile, runs, dist_km, dur_raw, u_raw, []
                )
                st.session_state["last_run_saved"] = {
                    "dist": dist_km, "dur": dur_raw, "xp": total_xp,
                    "is_pb": is_pb, "fresh": fresh, "unit": u_raw
                }
                # Clear bridge
                st.session_state["gps_bridge"] = ""
                st.rerun()
            else:
                st.session_state["gps_bridge"] = ""
        except Exception as e:
    ...
    st.caption("💡 Go outside first · Wait for accuracy <15m · Run stops and saves automatically when you press HALT")
