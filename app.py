...
def page_quests(user_id, profile, runs, c):
    ...
    for i, r in enumerate(RANKS):
        ...
        code = RANK_CODES[i]
        border = c["ACCENT"] if is_cur else (c["BORDER2"] if unlocked else c["BORDER2"]+"60")
        bg = "rgba(255,179,0,0.12)" if is_cur else "transparent"
        tc = c["ACCENT"] if is_cur else c["TEXT3"]
        op = "1" if unlocked else "0.3"
        now = f"<div style='font-size:9px;color:{c['ACCENT']};font-family:monospace;letter-spacing:1px;'>▲</div>" if is_cur else ""
        cells += f"""<td style='padding:3px 2px; opacity:{op};'>
            <div style='width:42px;text-align:center;padding:7px 2px 5px;border-radius:6px;
                border:2px solid {border};background:{bg};'>
                <div style='font-size:16px;line-height:1;'>{r["icon"]}</div>
                <div style='font-family:Orbitron,sans-serif;font-size:9px;font-weight:700;
                    color:{tc};margin-top:3px;letter-spacing:0.5px;'>{code}</div>
                {now}
            </div></td>"""
