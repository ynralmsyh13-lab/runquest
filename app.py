...
<div style="font-family:'Exo 2',sans-serif;">
  ...
  <div id="sum" style="display:none; background:{c['CARD']}; border:1.5px solid {c['BORDER2']}; border-radius:8px; padding:16px; position:relative; overflow:hidden;">
    ...
    <button onclick="gsave()" style="width:100%;padding:13px;border-radius:6px;border:2px solid {c['ACCENT']};cursor:pointer;background:rgba(255,179,0,0.12);color:{c['ACCENT']};font-family:'Orbitron',sans-serif;font-size:0.75rem;font-weight:800;letter-spacing:2px;">◈ NOTE YOUR DISTANCE & TIME ABOVE</button>
  </div>
</div>

<script>
var wId=null,tI=null,lP=null,tD=0,tS=0,paused=false,wl=null,unit='{unit_sel}';
var route=[],liveMap=null,liveMarker=null,sumMap=null;
...
function initLiveMap(lat,lon){{
    ...
    if(liveMap){{liveMap.remove();liveMap=null;}}
        ...
        container:'livemap',style:'https://tiles.openfreemap.org/styles/liberty',
        center:[lon,lat],zoom:17,pitch:50,bearing:0,antialias:true
    }});
    liveMap.addControl(new maplibregl.NavigationControl({{visualizePitch:true}}),'bottom-right');
    liveMap.on('load',function(){{
        var ls=liveMap.getStyle().layers,lid;
        for(var i=0;i<ls.length;i++){{if(ls[i].type==='symbol'&&ls[i].layout['text-field']){{lid=ls[i].id;break;}}}}
        liveMap.addLayer({{
            id:'3d-b',source:'composite','source-layer':'building',
            filter:['==','extrude','true'],type:'fill-extrusion',minzoom:13,
            paint:{{'fill-extrusion-color':'#ccc',
                'fill-extrusion-height':['interpolate',['linear'],['zoom'],13,0,13.05,['get','height']],
                'fill-extrusion-base':['interpolate',['linear'],['zoom'],13,0,13.05,['get','min_height']],
                'fill-extrusion-opacity':0.55}}
        }},lid);
        liveMap.addSource('lr',{{type:'geojson',data:{{type:'Feature',geometry:{{type:'LineString',coordinates:[[lon,lat]]}}}}}});
        liveMap.addLayer({{id:'ll',type:'line',source:'lr',
            layout:{{'line-join':'round','line-cap':'round'}},
            paint:{{'line-color':'{accent}','line-width':5,'line-opacity':0.95}}}});
        new maplibregl.Marker({{color:'#00C853'}}).setLngLat([lon,lat]).setPopup(new maplibregl.Popup().setHTML('<b>START</b>')).addTo(liveMap);
        var el=document.createElement('div');
        el.style.cssText='width:16px;height:16px;border-radius:50%;background:{accent};border:3px solid white;animation:pg 1.5s infinite;';
...
function updateLiveMap(lat,lon){{
    ...
    try{{
        var src=liveMap.getSource('lr');
        if(src){{
            var d=src._data||{{type:'Feature',geometry:{{type:'LineString',coordinates:[]}}}};
            d.geometry.coordinates.push([lon,lat]);
            src.setData(d);
        }}
        liveMap.easeTo({{center:[lon,lat],duration:800}});
    }}catch(e){{}}
}}
...
function initSumMap(){{
    ...
    setTimeout(function(){{
        ...
        var coords=route.map(p=>[p.lon,p.lat]);
                        sumMap=new maplibregl.Map({{
            container:'summap',style:'https://tiles.openfreemap.org/styles/liberty',
            center:[clon,clat],zoom:15,pitch:45,bearing:-10,antialias:true
        }});
        sumMap.on('load',function(){{
            var ls=sumMap.getStyle().layers,lid;
            for(var i=0;i<ls.length;i++){{if(ls[i].type==='symbol'&&ls[i].layout['text-field']){{lid=ls[i].id;break;}}}}
            sumMap.addLayer({{
                id:'3d-b',source:'composite','source-layer':'building',
                filter:['==','extrude','true'],type:'fill-extrusion',minzoom:13,
            ...
            sumMap.addSource('r',{{type:'geojson',data:{{type:'Feature',geometry:{{type:'LineString',coordinates:coords}}}}}});
            sumMap.addLayer({{id:'rl',type:'line',source:'r',
                layout:{{'line-join':'round','line-cap':'round'}},
                paint:{{'line-color':'{accent}','line-width':5,'line-opacity':0.95}}}});
            ...
            sumMap.fitBounds(bounds,{{padding:40,pitch:45,duration:1200}});
        }});
    }},400);
}}
...
function gst(){{
    ...
    if(tD > 0.0005 && tS > 0){{
        setStatus('<span style="color:{c["ACCENT"]};font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ SCROLL DOWN — ENTER DISTANCE & TIME — TAP SAVE ]</span>');
    }} else {{
        setStatus('<span style="color:#FF8C00;font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ NO DISTANCE RECORDED ]</span>');
    }}
}}

function gsave(){{
    setStatus('<span style="color:{c["ACCENT"]};font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ USE THE SAVE FORM BELOW ]</span>');
}}
</script>
