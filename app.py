...
function updateLiveMap(lat,lon){{
    ...
    if(livePoly){{
        ...
        var bounds=livePoly.getBounds();
        if(bounds.isValid())liveMap.fitBounds(bounds,{{padding:[30,30],maxZoom:18,animate:true}});
    }}
}}
function initSumMap(){{
...
Be specific, motivating, and concise. Under 250 words."""
        with st.spinner("🤖 ARIA is analyzing your run..."):
            try:
                ...
                data = resp.json()
                result = data["choices"][0]["message"]["content"] if "choices" in data else "Error generating debrief."
