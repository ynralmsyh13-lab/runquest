...
def page_quests(user_id, profile, runs, c):
    ...
    # SVG paths for each rank - unique geometric shapes
    RANK_SVGS = [
        # 1 Wanderer - circle with dot
        f'<svg width="26" height="26" viewBox="0 0 26 26"><circle cx="13" cy="13" r="10" stroke="CLRX" stroke-width="1.5" fill="FILX"/><circle cx="13" cy="13" r="4" fill="CLRX"/></svg>',
        # 2 Jogger - triangle
        f'<svg width="26" height="26" viewBox="0 0 26 26"><polygon points="13,2 24,22 2,22" stroke="CLRX" stroke-width="1.5" fill="FILX"/><polygon points="13,9 18,19 8,19" fill="CLRX"/></svg>',
        # 3 Pacer - diamond
        f'<svg width="26" height="26" viewBox="0 0 26 26"><rect x="5" y="5" width="16" height="16" stroke="CLRX" stroke-width="1.5" fill="FILX" transform="rotate(45 13 13)"/><rect x="9" y="9" width="8" height="8" fill="CLRX" transform="rotate(45 13 13)"/></svg>',
        # 4 Strider - heptagon
        f'<svg width="26" height="26" viewBox="0 0 26 26"><polygon points="13,2 20,7 23,15 18,23 8,23 3,15 6,7" stroke="CLRX" stroke-width="1.5" fill="FILX"/><circle cx="13" cy="13" r="3" fill="CLRX"/></svg>',
        # 5 Racer - star
        f'<svg width="26" height="26" viewBox="0 0 26 26"><polygon points="13,1 15.5,9.5 24,9.5 17.5,14.5 20,23 13,18 6,23 8.5,14.5 2,9.5 10.5,9.5" stroke="CLRX" stroke-width="1.5" fill="FILX"/></svg>',
        # 6 Sprinter - shield
        f'<svg width="26" height="26" viewBox="0 0 26 26"><path d="M13 2 L22 6 L22 16 L13 24 L4 16 L4 6 Z" stroke="CLRX" stroke-width="1.5" fill="FILX"/><path d="M13 7 L18 10 L18 15 L13 20 L8 15 L8 10 Z" fill="CLRX"/></svg>',
        # 7 Champion - triple ring
        f'<svg width="26" height="26" viewBox="0 0 26 26"><circle cx="13" cy="13" r="11" stroke="CLRX" stroke-width="1.5" fill="none"/><circle cx="13" cy="13" r="7" stroke="CLRX" stroke-width="1" fill="FILX"/><circle cx="13" cy="13" r="3" fill="CLRX"/></svg>',
        # 8 Legend - pentagon
        f'<svg width="26" height="26" viewBox="0 0 26 26"><path d="M13 2 L24 9 L20 22 L6 22 L2 9 Z" stroke="CLRX" stroke-width="1.5" fill="FILX"/><path d="M13 7 L18 11 L16 18 L10 18 L8 11 Z" fill="CLRX"/></svg>',
        # 9 Mythic - double star
        f'<svg width="26" height="26" viewBox="0 0 26 26"><polygon points="13,1 14.8,8.5 22,7 16.5,12 22,17 14.8,15.5 13,23 11.2,15.5 4,17 9.5,12 4,7 11.2,8.5" stroke="CLRX" stroke-width="1.2" fill="FILX"/><circle cx="13" cy="12" r="3" fill="CLRX"/></svg>',
        # 10 Transcendent - sun/mandala
        f'<svg width="26" height="26" viewBox="0 0 26 26"><circle cx="13" cy="13" r="9" stroke="CLRX" stroke-width="1.5" fill="FILX"/><circle cx="13" cy="13" r="4" fill="CLRX"/><line x1="13" y1="1" x2="13" y2="4" stroke="CLRX" stroke-width="1.5"/><line x1="13" y1="22" x2="13" y2="25" stroke="CLRX" stroke-width="1.5"/><line x1="1" y1="13" x2="4" y2="13" stroke="CLRX" stroke-width="1.5"/><line x1="22" y1="13" x2="25" y2="13" stroke="CLRX" stroke-width="1.5"/><line x1="4.5" y1="4.5" x2="6.6" y2="6.6" stroke="CLRX" stroke-width="1.5"/><line x1="19.4" y1="19.4" x2="21.5" y2="21.5" stroke="CLRX" stroke-width="1.5"/><line x1="21.5" y1="4.5" x2="19.4" y2="6.6" stroke="CLRX" stroke-width="1.5"/><line x1="6.6" y1="19.4" x2="4.5" y2="21.5" stroke="CLRX" stroke-width="1.5"/></svg>',
    ]

    RANK_NAMES = ["Wanderer","Jogger","Pacer","Strider","Racer","Sprinter","Champion","Legend","Mythic","Transcendent"]
    ...
    for i, r in enumerate(RANKS):
        ...
        with cols[i]:
            clr = c["ACCENT"] if is_cur else (c["TEXT3"] if unlocked else c["TEXT3"])
            fil = f"rgba(255,179,0,0.15)" if is_cur else ("rgba(255,179,0,0.05)" if unlocked else "rgba(90,74,42,0.05)")
            op = "1" if unlocked else "0.2"
            border = f"2px solid {c['ACCENT']}" if is_cur else f"1px solid {c['BORDER2'] if unlocked else c['BORDER2']+'40'}"
            svg = RANK_SVGS[i].replace("CLRX", clr).replace("FILX", fil)
            now_badge = f"<div style='font-size:8px;color:{c['ACCENT']};font-family:monospace;line-height:1;'>▲</div>" if is_cur else ""
            st.markdown(f"""
            <div title="{RANK_NAMES[i]} · LV.{r['level']} · {r['min_xp']} XP"
                style='text-align:center;padding:6px 2px 4px;border-radius:6px;
                border:{border};opacity:{op};cursor:pointer;
                {"background:rgba(255,179,0,0.1);" if is_cur else ""}'>
                {svg}{now_badge}
            </div>""", unsafe_allow_html=True)

    # Tap instruction
    st.markdown(f"<div style='font-family:Share Tech Mono,monospace; font-size:0.55rem; color:{c['TEXT3']}; letter-spacing:1px; margin-top:6px; text-align:center;'>LONG PRESS ICON FOR RANK NAME</div>", unsafe_allow_html=True)
