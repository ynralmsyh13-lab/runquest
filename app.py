...
def render_sidebar(profile, page, c):
    with st.sidebar:
        ...
        # Logo
        st.markdown(f"""
        <div style='padding:24px 20px 18px; border-bottom:2px solid {c["BORDER2"]}60;
            background:linear-gradient(135deg, {"rgba(255,179,0,0.05)" if c["dark_mode"] else "rgba(192,120,0,0.05)"} 0%, transparent 100%);'>
            <div style='font-family:Share Tech Mono,monospace; font-size:0.5rem; color:{c["TEXT3"]};
                letter-spacing:4px; margin-bottom:10px; display:flex; align-items:center; gap:6px;'>
                <span style='display:inline-block; width:20px; height:1px; background:{c["ACCENT"]};'></span>
                SYSTEM ACTIVE
                <span style='display:inline-block; width:20px; height:1px; background:{c["ACCENT"]};'></span>
            </div>
            <div style='font-family:Orbitron,sans-serif; font-size:1.65rem; font-weight:900; letter-spacing:5px; line-height:1.1;
            ...
            <div style='font-family:Share Tech Mono,monospace; font-size:0.48rem; color:{c["TEXT3"]};
                letter-spacing:4px; margin-top:6px;'>PERFORMANCE OS v2.0</div>
            <div style='margin-top:10px; height:1px;
                background:linear-gradient(90deg, {c["ACCENT"]}80, {c["ACCENT2"]}60, transparent);'></div>
        </div>
        """, unsafe_allow_html=True)
