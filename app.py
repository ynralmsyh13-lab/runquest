...
def page_ai_coach(user_id, profile, runs, unit, c):
    ...
    if not CLAUDE_API_KEY:
        st.error("⚠ Add CLAUDE_API_KEY to Streamlit Secrets to enable AI Coach.")
        st.code('CLAUDE_API_KEY = "sk-ant-..."')
        return

    # Build context from user data
    xp = profile.get("xp", 0)
    rank_name, rank_lv = get_rank(xp)
    total_runs = len(runs)
    ...
    def ask_claude(prompt, system=""):
        ...
        try:
            resp = requests.post(
                ...
                json={"model": "claude-haiku-4-5-20251001", "max_tokens": 400, "system": full_system, "messages": [{"role": "user", "content": prompt}]},
                timeout=30
            )
            data = resp.json()
            return data["content"][0]["text"] if "content" in data else f"Error: {data.get('error', {}).get('message', 'Unknown error')}"
        except Exception as e:
