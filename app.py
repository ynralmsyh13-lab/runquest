...
def render_route_map(route, c, height=300):
    ...
    coords_js = json.dumps([[p["lon"], p["lat"]] for p in route])
    ...
    <div id='rmap' style='width:100%;height:{height}px;border-radius:8px;overflow:hidden;border:1.5px solid {border};'></div>
    <script>
    var map = new maplibregl.Map({{
        container:'rmap',
        style:'https://tiles.openfreemap.org/styles/liberty',
        center:[{clon},{clat}],
        zoom:15, pitch:50, bearing:-20, antialias:true
    }});
    map.addControl(new maplibregl.NavigationControl({{visualizePitch:true}}),'bottom-right');
    map.on('load',function(){{
        var layers=map.getStyle().layers, lid;
        for(var i=0;i<layers.length;i++){{
            if(layers[i].type==='symbol'&&layers[i].layout['text-field']){{lid=layers[i].id;break;}}
        }}
        if(lid){{
            map.addLayer({{
        ...
        map.addSource('route',{{type:'geojson',data:{{type:'Feature',geometry:{{type:'LineString',coordinates:{coords_js}}}}}}});
        map.addLayer({{id:'route-line',type:'line',source:'route',
            layout:{{'line-join':'round','line-cap':'round'}},
            paint:{{'line-color':'{accent}','line-width':5,'line-opacity':0.95}}}});
        new maplibregl.Marker({{color:'#00C853'}}).setLngLat([{start_p["lon"]},{start_p["lat"]}]).setPopup(new maplibregl.Popup().setHTML('<b>START</b>')).addTo(map);
        new maplibregl.Marker({{color:'#FF4444'}}).setLngLat([{end_p["lon"]},{end_p["lat"]}]).setPopup(new maplibregl.Popup().setHTML('<b>FINISH</b>')).addTo(map);
        var coords={coords_js};
        var bounds=coords.reduce(function(b,c2){{return b.extend(c2);}},new maplibregl.LngLatBounds(coords[0],coords[0]));
        map.fitBounds(bounds,{{padding:50,pitch:50,duration:1500}});
    }});
    </script>
...
<div style="font-family:'Exo 2',sans-serif;">
  ...
  <div id="map-wrap" style="display:none;margin-bottom:12px;border-radius:8px;overflow:hidden;border:1.5px solid {border2};">
    ...
    <div id="livemap" class="ml-map"></div>
  </div>
  ...
  <div id="sum" style="display:none;background:{card};border:1.5px solid {border2};border-radius:8px;padding:16px;position:relative;overflow:hidden;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
      <div style="text-align:center;"><div style="font-family:'Exo 2',sans-serif;font-size:0.7rem;font-weight:700;color:{text3};margin-bottom:4px;">DISTANCE</div><div id="sd" style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{accent};"></div></div>
      <div style="text-align:center;"><div style="font-family:'Exo 2',sans-serif;font-size:0.7rem;font-weight:700;color:{text3};margin-bottom:4px;">DURATION</div><div id="st2" style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{text_c};"></div></div>
      <div style="text-align:center;"><div style="font-family:'Exo 2',sans-serif;font-size:0.7rem;font-weight:700;color:{text3};margin-bottom:4px;">AVG PACE</div><div id="sp" style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{text_c};"></div></div>
      <div style="text-align:center;"><div style="font-family:'Exo 2',sans-serif;font-size:0.7rem;font-weight:700;color:{text3};margin-bottom:4px;">STATUS</div><div style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{accent};">DONE ✓</div></div>
    </div>
    <div id="sum-map-wrap" style="border-radius:6px;overflow:hidden;border:1px solid {border2};margin-bottom:14px;"><div id="summap" class="ml-map"></div></div>
...
var route=[],liveMap=null,liveMarker=null,routeCoords=[],sumMap=null;
...
function initLiveMap(lat,lon){{
    ...
    liveMap=new maplibregl.Map({{container:'livemap',style:STYLE,center:[lon,lat],zoom:17,pitch:45,antialias:true}});
    liveMap.addControl(new maplibregl.NavigationControl({{visualizePitch:true}}),'bottom-right');
    routeCoords=[[lon,lat]];
    liveMap.on('load',function(){{
        liveMap.addSource('route',{{type:'geojson',data:{{type:'Feature',geometry:{{type:'LineString',coordinates:[[lon,lat]]}}}}}});
        liveMap.addLayer({{id:'route-line',type:'line',source:'route',layout:{{'line-join':'round','line-cap':'round'}},paint:{{'line-color':'{accent}','line-width':5,'line-opacity':0.95}}}});
        new maplibregl.Marker({{color:'#00C853'}}).setLngLat([lon,lat]).setPopup(new maplibregl.Popup().setHTML('<b>START</b>')).addTo(liveMap);
        var el=document.createElement('div');
        el.style.cssText='width:16px;height:16px;border-radius:50%;background:{accent};border:3px solid white;box-shadow:0 0 10px {accent};animation:pg 1.5s infinite;';
        liveMarker=new maplibregl.Marker({{element:el}}).setLngLat([lon,lat]).addTo(liveMap);
    }});
}}
function updateLiveMap(lat,lon){{
    if(!liveMap||!liveMarker)return;
    liveMarker.setLngLat([lon,lat]);
    routeCoords.push([lon,lat]);
    try{{var src=liveMap.getSource('route');if(src)src.setData({{type:'Feature',geometry:{{type:'LineString',coordinates:routeCoords}}}});liveMap.easeTo({{center:[lon,lat],duration:800}});}}catch(e){{}}
}}
function initSumMap(){{
    if(route.length<2)return;
    setTimeout(function(){{
        if(sumMap){{sumMap.remove();sumMap=null;}}
        var coords=route.map(p=>[p.lon,p.lat]);
        var clat=route.reduce((s,p)=>s+p.lat,0)/route.length;
        var clon=route.reduce((s,p)=>s+p.lon,0)/route.length;
        sumMap=new maplibregl.Map({{container:'summap',style:STYLE,center:[clon,clat],zoom:15,pitch:45,antialias:true}});
        sumMap.on('load',function(){{
            sumMap.addSource('sr',{{type:'geojson',data:{{type:'Feature',geometry:{{type:'LineString',coordinates:coords}}}}}});
            sumMap.addLayer({{id:'sl',type:'line',source:'sr',layout:{{'line-join':'round','line-cap':'round'}},paint:{{'line-color':'{accent}','line-width':5,'line-opacity':0.95}}}});
            new maplibregl.Marker({{color:'#00C853'}}).setLngLat(coords[0]).addTo(sumMap);
            new maplibregl.Marker({{color:'#FF4444'}}).setLngLat(coords[coords.length-1]).addTo(sumMap);
            var bounds=coords.reduce((b,c)=>b.extend(c),new maplibregl.LngLatBounds(coords[0],coords[0]));
            sumMap.fitBounds(bounds,{{padding:40,duration:1200}});
        }});
    }},400);
}}
function gs(){{
    if(!navigator.geolocation){{setStatus('<span style="color:#FF5555;">❌ GPS NOT AVAILABLE</span>');return;}}
    tD=0;tS=0;lP=null;paused=false;route=[];routeCoords=[];
