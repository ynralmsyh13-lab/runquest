...
def render_sidebar(profile, page, c):
    with st.sidebar:
        ...
        # Logo
        logo_run_color = c["ACCENT"]
        logo_quest_color = c["TEXT"]
        logo_bg = "#0F1218" if c["dark_mode"] else "#FFF3D4"
        logo_border = c["ACCENT"]
        logo_sub_color = c["TEXT3"]
        logo_dash_color = c["TEXT3"]

        st.markdown(f"""
        <div style='padding:22px 18px 16px;
            background:{logo_bg};
            border-bottom:2px solid {logo_border};
            ...
            <div style='font-family:Share Tech Mono,monospace; font-size:0.52rem;
                color:{logo_dash_color}; letter-spacing:4px; margin-bottom:12px;'>
            ...
            <div style='font-family:Orbitron,sans-serif; font-size:1.75rem; font-weight:900;
                ...
                <span style='color:{logo_run_color};'>RUN</span><span style='color:{logo_quest_color};'>QUEST</span>
            </div>
            <div style='font-family:Share Tech Mono,monospace; font-size:0.5rem;
                color:{logo_sub_color}; letter-spacing:4px; margin-bottom:10px;'>PERFORMANCE OS v2.0</div>
            <div style='height:1px; background:linear-gradient(90deg, {c["ACCENT"]}, transparent);'></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Nav buttons
        st.markdown(f"""
        <div style='padding:8px 0 2px; border-bottom:1px solid {c["BORDER2"]}40;'>
            <div style='padding:4px 16px 6px; font-family:Share Tech Mono,monospace;
                font-size:0.52rem; color:{c["TEXT3"]}; letter-spacing:4px;'>◈ NAVIGATE</div>
        </div>
        """, unsafe_allow_html=True)

        NAV_ITEMS = [
            ("⚡", "Dashboard",   "Home & overview"),
            ("📍", "GPS Tracker", "Live run tracking"),
            ("✍️", "Manual Log",  "Log from device"),
            ("📋", "History",     "Past missions"),
            ("🎯", "Quests",      "RPG progression"),
            ("📈", "Stats",       "Data analysis"),
            ("⚙️", "Settings",    "Configuration"),
        ]

        st.markdown("<div class='sidebar-nav'>", unsafe_allow_html=True)
        for icon, name, hint in NAV_ITEMS:
            is_active = page == name
            st.markdown(f"<div class='{'sidebar-active' if is_active else ''}'>", unsafe_allow_html=True)
            if st.button(f"{icon}  {name}", key=f"snav_{name}", help=hint):
