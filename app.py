    ...
    with st.sidebar:
        ...
        <div style='padding:12px 10px 10px; background:{logo_bg}; border-bottom:2px solid {c["ACCENT"]}; position:relative;'>
            ...
            <div style='border:1px solid {c["ACCENT"]}40; padding:8px 8px; position:relative; border-radius:2px;'>
                ...
                <div style='font-family:Orbitron,sans-serif; font-weight:900; letter-spacing:2px; line-height:1.1; margin-bottom:5px;'>
                    <div style='font-size:1.6rem; color:{c["ACCENT"]};'>RUN</div>
                    <div style='font-size:1.6rem; color:{c["TEXT"]};'>QUEST</div>
                </div>
...
def render_route_map(route, c, height=300):
    ...
    for p in route:
        if isinstance(p, dict):
            norm.append({"lat": p.get("lat", p.get("latitude", 0)), "lon": p.get("lon", p.get("longitude", 0))})
        elif isinstance(p, (list, tuple)) and len(p) >= 2:
            norm.append({"lat": p[0], "lon": p[1]})
...
var startTime=null,pausedAt=null,totalPaused=0;
var OSM='{osm_tile}';
...
function getElapsed(){{
    ...
    var now=Date.now();
    var elapsed=Math.floor((now-startTime-totalPaused)/1000);
    ...
    try{{
        if('wakeLock'in navigator){{
            ...
            wl.addEventListener('release',function(){{
                if(!paused&&startTime){{rwl();}}
            }});
        }}
    }}catch(e){{document.getElementById('wl-warn').style.display='block';}}
}}
document.addEventListener('visibilitychange',function(){{
    if(document.visibilityState==='visible'&&!paused&&startTime){{
        rwl();
        var el=getElapsed();
        document.getElementById('tv').textContent=ft(el);
        document.getElementById('pv').textContent=fp(el,tD);
    }}
}});
...
function initLiveMap(lat,lon){{
    ...
    livePoly=L.polyline([],{{color:'{accent}',weight:6,opacity:0.95,lineCap:'round',lineJoin:'round'}}).addTo(liveMap);
...
function gs(){{
    ...
    tD=0;lP=null;paused=false;route=[];
    startTime=Date.now();totalPaused=0;pausedAt=null;
...
function gp(){{
    ...
    document.getElementById('b-resume').style.display='flex';
    setStatus('<span style="color:#FF8C00;font-family:Share Tech Mono,monospace;font-size:0.75rem;letter-spacing:2px;">⏸ PAUSED</span>');
}}
function gr(){{
    if(pausedAt){{totalPaused+=Date.now()-pausedAt;pausedAt=null;}}
    paused=false;lP=null;
    document.getElementById('b-resume').style.display='none';
    document.getElementById('b-pause').style.display='flex';
    setStatus('<span style="color:{accent};font-family:Share Tech Mono,monospace;font-size:0.75rem;letter-spacing:2px;">● TRACKING ACTIVE</span>');
}}
function gst(){{
    var finalTime=getElapsed();
    ...
    document.getElementById('st2').textContent=ft(finalTime);
    document.getElementById('sp').textContent=fp(finalTime,tD)+'/'+unit;
    document.getElementById('fd').value=tD.toFixed(4);
    document.getElementById('ft').value=finalTime;
...
def page_history(user_id, runs, unit, c):
    ...
    for r in runs:
        ...
        # Show route map inline if available
        if has_route:
            # Normalize route
            norm = []
            for p in route:
                if isinstance(p, dict):
                    ...
                    norm.append({"lat": p[0], "lon": p[1]})
            if len(norm) >= 2:
                render_route_map(norm, c, height=220)
...
def page_settings(user_id, profile, c):
    ...
    with col_x:
        if st.button("📤 Export Run Data (.xlsx)", use_container_width=True):
            runs_data = get_runs(user_id)
            if not runs_data:
                st.warning("No runs to export yet!")
            else:
                import io
                ...
                header_fill = PatternFill("solid", start_color="1A1200")
                accent_fill = PatternFill("solid", start_color="332200")
                alt_fill    = PatternFill("solid", start_color="0E0E0E")
                gold = "FFB300"
                white = "F0E6C8"
                gray = "888888"

                headers = ["#", "Date", "Distance (km)", "Duration", "Avg Pace (min/km)", "XP Earned", "Personal Best", "Route Points"]
                col_widths = [5, 14, 16, 12, 20, 12, 14, 14]
                ...
                for ri, r in enumerate(runs_data, 2):
                    dist = round(r.get("distance_km", 0), 3)
                    secs = r.get("duration_seconds", 0)
                    h_, m_, s_ = secs//3600, (secs%3600)//60, secs%60
                    dur_str = f"{h_:02d}:{m_:02d}:{s_:02d}"
                    pace_sec = (secs / dist) if dist > 0 else 0
                    pace_str = f"{int(pace_sec//60)}:{int(pace_sec%60):02d}"
                    date_str = r.get("started_at", "")[:10]
                    xp = r.get("xp_earned", 0)
                    pb = "YES" if r.get("is_pb") else "—"
                    route_pts = len(r.get("route") or [])
                    fill = accent_fill if ri % 2 == 0 else alt_fill
                    row_data = [ri-1, date_str, dist, dur_str, pace_str, xp, pb, route_pts]
                    for ci, val in enumerate(row_data, 1):
                        cell = ws1.cell(row=ri, column=ci, value=val)
                        cell.font = Font(color=white if ci != 1 else gold, name="Arial", size=10)
                        cell.fill = fill
                        cell.alignment = Alignment(horizontal="center", vertical="center")
                ...
                total_km   = sum(r.get("distance_km",0) for r in runs_data)
                total_secs = sum(r.get("duration_seconds",0) for r in runs_data)
                total_xp   = sum(r.get("xp_earned",0) for r in runs_data)
                avg_pace_s = (total_secs / total_km) if total_km else 0
                best_pace_s= min((r["duration_seconds"]/r["distance_km"] for r in runs_data if r.get("distance_km",0)>0), default=0)
                pb_count   = sum(1 for r in runs_data if r.get("is_pb"))
                ...
                stats = [
                    ...
                    ("Total Time", f"{total_secs//3600}h {(total_secs%3600)//60}m"),
                    ("Total XP Earned", total_xp),
                    ("Rank", f"{rank['title']} LV.{rank['level']}"),
                    ("Personal Bests", pb_count),
                    ("", ""),
                    ("PACE STATS", ""),
                    ("Avg Pace", f"{int(avg_pace_s//60)}:{int(avg_pace_s%60):02d} /km"),
                    ("Best Pace", f"{int(best_pace_s//60)}:{int(best_pace_s%60):02d} /km"),
                    ("Avg Distance/Run (km)", round(total_km/total_runs, 2) if total_runs else 0),
                ]

                for ri, (label, val) in enumerate(stats, 1):
                    la = ws2.cell(row=ri, column=1, value=label)
                    vb = ws2.cell(row=ri, column=2, value=val)
                    if label in ("RUNQUEST EXPORT", "OPERATIVE STATS", "PACE STATS"):
                        la.font = Font(bold=True, color=gold, name="Arial", size=11)
                        la.fill = header_fill
                        vb.fill = header_fill
                    elif label:
                        la.font = Font(color=gray, name="Arial", size=10)
                        vb.font = Font(bold=True, color=white, name="Arial", size=10)
                    la.alignment = Alignment(vertical="center")
                    vb.alignment = Alignment(horizontal="right", vertical="center")
...
def page_challenges(user_id, profile, runs, unit, c):
    import datetime
    st.markdown("<div class='page-title'>◈ CHALLENGES</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>TIME-LIMITED EVENTS — EARN BONUS XP</div>", unsafe_allow_html=True)

    now = datetime.datetime.utcnow()
    week_start = now - datetime.timedelta(days=now.weekday())
    week_end = week_start + datetime.timedelta(days=7)
    month_end = (now.replace(day=1) + datetime.timedelta(days=32)).replace(day=1)
    days_left_week = (week_end - now).days
    days_left_month = (month_end - now).days

    week_runs = [r for r in runs if r.get("started_at","") >= week_start.strftime("%Y-%m-%d")]
    week_dist = sum(r["distance_km"] for r in week_runs)
    week_count = len(week_runs)
    month_runs = [r for r in runs if r.get("started_at","")[:7] == now.strftime("%Y-%m")]
    month_dist = sum(r["distance_km"] for r in month_runs)
    best_pace = min((r["duration_seconds"]/r["distance_km"] for r in week_runs if r.get("distance_km",0)>0), default=999)

    CHALLENGES = [
        {"id":"w_dist","period":"WEEKLY","title":"Distance Warrior","desc":"Run 20km this week","target":20,"current":round(week_dist,2),"unit":"km","xp":150,"days":days_left_week,"color":"#4FC3F7"},
        {"id":"w_runs","period":"WEEKLY","title":"Consistency King","desc":"Complete 4 runs this week","target":4,"current":week_count,"unit":"runs","xp":100,"days":days_left_week,"color":"#81C784"},
        {"id":"w_pace","period":"WEEKLY","title":"Speed Demon","desc":"Achieve sub 5:30/km pace in any run","target":330,"current":best_pace,"unit":"pace","xp":120,"days":days_left_week,"color":"#FFB74D","invert":True},
        {"id":"m_dist","period":"MONTHLY","title":"Century Challenger","desc":"Run 80km this month","target":80,"current":round(month_dist,2),"unit":"km","xp":400,"days":days_left_month,"color":"#CE93D8"},
        {"id":"m_runs","period":"MONTHLY","title":"Iron Operative","desc":"Complete 20 runs this month","target":20,"current":len(month_runs),"unit":"runs","xp":300,"days":days_left_month,"color":"#F48FB1"},
        {"id":"m_pb","period":"MONTHLY","title":"Personal Evolution","desc":"Set 3 personal bests this month","target":3,"current":sum(1 for r in month_runs if r.get("is_pb")),"unit":"PBs","xp":250,"days":days_left_month,"color":"#FFB300"},
    ]

    for period in ["WEEKLY","MONTHLY"]:
        period_challenges = [ch for ch in CHALLENGES if ch["period"]==period]
        label = f"◈ {period} CHALLENGES"
        sublabel = f"{'Resets in '+str(days_left_week)+' days' if period=='WEEKLY' else 'Resets in '+str(days_left_month)+' days'}"
        st.markdown(f"""
        <div style='display:flex;justify-content:space-between;align-items:center;margin:16px 0 10px;'>
            <div style='font-family:Orbitron,sans-serif;font-size:0.82rem;font-weight:700;color:{c["ACCENT"]};letter-spacing:2px;'>{label}</div>
            <div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;color:{c["TEXT3"]};letter-spacing:1px;'>⏱ {sublabel}</div>
        </div>""", unsafe_allow_html=True)

        for ch in period_challenges:
            inv = ch.get("invert", False)
            if inv:
                pct = min(100, max(0, int((1 - (ch["current"]-ch["target"])/(999-ch["target"])) * 100))) if ch["current"]<999 else 0
                done = ch["current"] <= ch["target"] and ch["current"] < 999
                cur_disp = f"{int(ch['current']//60)}:{int(ch['current']%60):02d}/km" if ch["current"]<999 else "--"
                tgt_disp = f"{int(ch['target']//60)}:{int(ch['target']%60):02d}/km"
            else:
                pct = min(100, int(ch["current"]/ch["target"]*100)) if ch["target"]>0 else 0
                done = ch["current"] >= ch["target"]
                cur_disp = str(ch["current"])
                tgt_disp = str(ch["target"])
            bar_color = "#00C853" if done else ch["color"]
            status_text = "✓ COMPLETE" if done else f"{cur_disp} / {tgt_disp} {ch['unit']}"
            status_color = "#00C853" if done else c["TEXT2"]
            st.markdown(f"""
            <div style='background:{c["CARD"]};border:1.5px solid {"#00C85340" if done else c["BORDER2"]};border-radius:10px;
                padding:16px;margin-bottom:10px;position:relative;overflow:hidden;'>
                <div style='position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,{ch["color"]},transparent);'></div>
                <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;'>
                    <div>
                        <div style='font-family:Orbitron,sans-serif;font-size:0.85rem;font-weight:700;color:{c["TEXT"]};'>{ch["title"]}</div>
                        <div style='font-family:Exo 2,sans-serif;font-size:0.8rem;color:{c["TEXT3"]};margin-top:2px;'>{ch["desc"]}</div>
                    </div>
                    <div style='background:rgba(255,179,0,0.12);border:1px solid {c["BORDER2"]};border-radius:4px;
                        padding:4px 10px;font-family:Orbitron,sans-serif;font-size:0.68rem;font-weight:800;color:{c["ACCENT"]};white-space:nowrap;margin-left:10px;'>
                        +{ch["xp"]} XP</div>
                </div>
                <div style='display:flex;justify-content:space-between;font-family:Share Tech Mono,monospace;
                    ...
                    <span style='color:{c["TEXT3"]};'>{ch["days"]}d left</span>
                </div>
                ...
                <div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};margin-top:4px;text-align:right;'>{pct}%</div>
            </div>""", unsafe_allow_html=True)


def page_leaderboard(user_id, profile, runs, unit, c):
    import datetime
    st.markdown("<div class='page-title'>◈ LEADERBOARD</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>GLOBAL WEEKLY XP RANKINGS</div>", unsafe_allow_html=True)
    ...
    # Register/update user on leaderboard
    try:
        now = datetime.datetime.utcnow()
        week = now.strftime("%Y-W%W")
        week_runs = [r for r in runs if r.get("started_at","") >= (now - datetime.timedelta(days=now.weekday())).strftime("%Y-%m-%d")]
        week_xp = sum(r.get("xp_earned",0) for r in week_runs)
        week_dist = round(sum(r["distance_km"] for r in week_runs), 2)
        sb.table("leaderboard").upsert({
            "user_id": user_id, "username": username,
            "week": week, "week_xp": week_xp,
            "total_xp": xp, "rank_title": rank["title"],
            "rank_level": rank["level"], "week_dist": week_dist
        }).execute()
    except: pass
    ...
    try:
        now = datetime.datetime.utcnow()
        week = now.strftime("%Y-W%W")
        res = sb.table("leaderboard").select("*").eq("week", week).order("week_xp", desc=True).limit(20).execute()
        board = res.data or []
    except:
        board = []
    ...
    # My position
    my_pos = next((i+1 for i,r in enumerate(board) if r.get("user_id")==user_id), None)
    if my_pos:
        st.markdown(f"""
        <div style='background:{c["CARD"]};border:2px solid {c["ACCENT"]}60;border-radius:10px;padding:14px 16px;margin-bottom:16px;position:relative;overflow:hidden;'>
            <div style='position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,{c["ACCENT"]},transparent);'></div>
            <div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;color:{c["TEXT3"]};letter-spacing:2px;margin-bottom:4px;'>YOUR POSITION THIS WEEK</div>
            <div style='font-family:Orbitron,sans-serif;font-size:1.4rem;font-weight:900;color:{c["ACCENT"]};'>#{my_pos}</div>
        </div>""", unsafe_allow_html=True)

    if not board:
        st.info("No leaderboard data yet. Complete runs to appear here!")
        return

    medals = {1:"◈ GOLD", 2:"◈ SILVER", 3:"◈ BRONZE"}
    medal_colors = {1:"#FFB300", 2:"#B0BEC5", 3:"#CD7F32"}

    for i, entry in enumerate(board):
        pos = i + 1
        is_me = entry.get("user_id") == user_id
        rl = min(max(entry.get("rank_level",1)-1, 0), 9)
        sym = RANK_SYMBOLS[rl]
        medal_col = medal_colors.get(pos, c["TEXT3"])
        border = f"2px solid {c['ACCENT']}" if is_me else f"1px solid {c['BORDER2']}"
        bg = f"rgba(255,179,0,0.06)" if is_me else "transparent"
        st.markdown(f"""
        <div style='background:{c["CARD"]};border:{border};border-radius:8px;padding:12px 14px;
            margin-bottom:6px;{f"background:{bg};" if is_me else ""}display:flex;align-items:center;gap:12px;'>
            <div style='font-family:Orbitron,sans-serif;font-size:0.9rem;font-weight:900;
                color:{medal_col};min-width:32px;'>{medals.get(pos,f"#{pos}")}</div>
            <div style='width:32px;height:32px;border:1.5px solid {c["ACCENT"] if is_me else c["BORDER2"]};
                border-radius:6px;display:flex;align-items:center;justify-content:center;
                font-size:1rem;color:{c["ACCENT"]};flex-shrink:0;background:rgba(255,179,0,0.08);'>{sym}</div>
            <div style='flex:1;'>
                <div style='font-family:Exo 2,sans-serif;font-size:0.9rem;font-weight:700;color:{c["ACCENT"] if is_me else c["TEXT"]};'>
                    {entry.get("username","UNKNOWN")} {"← YOU" if is_me else ""}</div>
                <div style='font-family:Share Tech Mono,monospace;font-size:0.58rem;color:{c["TEXT3"]};'>
                    {entry.get("rank_title","?")} · {entry.get("week_dist",0):.1f}km</div>
            </div>
            <div style='font-family:Orbitron,sans-serif;font-size:0.85rem;font-weight:900;color:{c["ACCENT"]};'>
                {entry.get("week_xp",0)} XP</div>
        </div>""", unsafe_allow_html=True)

    st.markdown(f"<div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c['TEXT3']};text-align:center;margin-top:8px;letter-spacing:1px;'>RESETS EVERY MONDAY · TOP 20 SHOWN</div>", unsafe_allow_html=True)


def page_heatmap(user_id, runs, c):
    import datetime
    st.markdown("<div class='page-title'>◈ ACTIVITY MAP</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>YOUR YEAR IN RUNNING</div>", unsafe_allow_html=True)
    ...
    for r in runs:
        ...
        day_map[d] = day_map.get(d, 0) + r.get("distance_km", 0)

    # Stats
    total_days = len(day_map)
    total_km = sum(day_map.values())
    max_day = max(day_map.values()) if day_map else 1
    longest_streak = 0; cur_streak = 0
    for i in range(365):
        d = (now - datetime.timedelta(days=i)).strftime("%Y-%m-%d")
        if day_map.get(d, 0) > 0: cur_streak += 1; longest_streak = max(longest_streak, cur_streak)
        else: cur_streak = 0

    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown(f"""<div style='background:{c["CARD"]};border:1.5px solid {c["BORDER2"]};border-radius:8px;padding:14px;text-align:center;'>
            <div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};letter-spacing:2px;margin-bottom:4px;'>ACTIVE DAYS</div>
            <div style='font-family:Orbitron,sans-serif;font-size:1.4rem;font-weight:900;color:{c["ACCENT"]};'>{total_days}</div></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div style='background:{c["CARD"]};border:1.5px solid {c["BORDER2"]};border-radius:8px;padding:14px;text-align:center;'>
            <div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};letter-spacing:2px;margin-bottom:4px;'>TOTAL KM</div>
            <div style='font-family:Orbitron,sans-serif;font-size:1.4rem;font-weight:900;color:{c["ACCENT"]};'>{total_km:.1f}</div></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div style='background:{c["CARD"]};border:1.5px solid {c["BORDER2"]};border-radius:8px;padding:14px;text-align:center;'>
            <div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};letter-spacing:2px;margin-bottom:4px;'>BEST STREAK</div>
            <div style='font-family:Orbitron,sans-serif;font-size:1.4rem;font-weight:900;color:{c["ACCENT"]};'>{longest_streak}d</div></div>""", unsafe_allow_html=True)

    st.markdown(f"<div style='font-family:Orbitron,sans-serif;font-size:0.78rem;font-weight:700;color:{c['ACCENT']};letter-spacing:2px;margin:18px 0 10px;'>◈ LAST 52 WEEKS</div>", unsafe_allow_html=True)
    ...
    for w in range(51, -1, -1):
        week_days = []
        for d in range(7):
            day = now - datetime.timedelta(weeks=w, days=now.weekday()-d)
            ds = day.strftime("%Y-%m-%d")
            km = day_map.get(ds, 0)
            week_days.append((ds, km))
    ...
    for wi, week in enumerate(weeks):
        m = week[0][0][5:7]
        if m != prev_month:
            month_labels.append((wi, datetime.datetime.strptime(week[0][0], "%Y-%m-%d").strftime("%b")))
            prev_month = m

    def km_to_color(km, dark):
        if km == 0: return "#1A1A1A" if dark else "#EEEEEE"
        intensity = min(km / max(max_day, 1), 1)
        if intensity < 0.25: return "#2D1F00" if dark else "#FFF3CC"
        elif intensity < 0.5: return "#7A4A00" if dark else "#FFD166"
        elif intensity < 0.75: return "#CC8800" if dark else "#FF9900"
        else: return "#FFB300" if dark else "#CC6600"

    dark = c["dark_mode"]
    cells_html = "<div style='overflow-x:auto;'><div style='display:inline-flex;flex-direction:column;gap:3px;'>"
    # Month labels row
    cells_html += "<div style='display:flex;gap:3px;margin-bottom:2px;'>"
    label_map = {wi: lbl for wi, lbl in month_labels}
    for wi in range(len(weeks)):
        lbl = label_map.get(wi, "")
        lbl_char = lbl[:1] if lbl else ""
        t3 = c["TEXT3"]
        cells_html += f"<div style='width:12px;font-family:Share Tech Mono,monospace;font-size:8px;color:{t3};text-align:center;'>{lbl_char}</div>"
    cells_html += "</div>"
    # Day rows
    DAY_LABELS = ["M","","W","","F","","S"]
    for d in range(7):
        day_lbl = DAY_LABELS[d]
        cells_html += f"<div style='display:flex;gap:3px;align-items:center;'>"
        for wi, week in enumerate(weeks):
            ds, km = week[d]
            col = km_to_color(km, dark)
            tip = f"{ds}: {km:.1f}km" if km > 0 else ds
            cells_html += f"<div title='{tip}' style='width:12px;height:12px;border-radius:2px;background:{col};flex-shrink:0;'></div>"
        cells_html += "</div>"
    cells_html += "</div></div>"

    t3 = c["TEXT3"]
    cells_html += f"<div style='display:flex;align-items:center;gap:6px;margin-top:10px;'>"
    cells_html += f"<span style='font-family:Share Tech Mono,monospace;font-size:9px;color:{t3};'>Less</span>"
    for km in [0, max_day*0.25, max_day*0.5, max_day*0.75, max_day]:
        cells_html += f"<div style='width:12px;height:12px;border-radius:2px;background:{km_to_color(km,dark)};'></div>"
    cells_html += f"<span style='font-family:Share Tech Mono,monospace;font-size:9px;color:{t3};'>More</span></div>"

    st.markdown(cells_html, unsafe_allow_html=True)


def page_route_gallery(user_id, runs, unit, c):
    st.markdown("<div class='page-title'>◈ ROUTE GALLERY</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>ALL YOUR ROUTES — YOUR RUNNING HISTORY</div>", unsafe_allow_html=True)

    routes = []
    for r in runs:
        raw = r.get("route") or []
        if not raw or len(raw) < 2: continue
        # Normalize: handle both {"lat":x,"lon":y} and [lat,lon] formats
        normalized = []
        for p in raw:
            if isinstance(p, dict):
                normalized.append({"lat": p.get("lat", p.get("latitude", 0)), "lon": p.get("lon", p.get("longitude", 0))})
            elif isinstance(p, (list, tuple)) and len(p) >= 2:
                normalized.append({"lat": p[0], "lon": p[1]})
        if len(normalized) >= 2:
            routes.append({**r, "route": normalized})
    if not routes:
        st.info("No GPS routes saved yet. Use GPS Tracker to record your runs!")
        return

    st.markdown(f"<div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;color:{c['TEXT3']};letter-spacing:2px;margin-bottom:14px;'>{len(routes)} ROUTES RECORDED</div>", unsafe_allow_html=True)

    cols = st.columns(2)
    for i, r in enumerate(routes):
        with cols[i % 2]:
            route = r.get("route", [])
            dist = format_distance(r["distance_km"], unit)
            dur = format_time(r["duration_seconds"])
            pace = format_pace(r["duration_seconds"], r["distance_km"], unit)
            date = r.get("started_at","")[:10]
            is_pb = r.get("is_pb", False)

            # Mini map using Leaflet in component
            lats = [p["lat"] for p in route]
            lons = [p["lon"] for p in route]
            clat = sum(lats)/len(lats)
            clon = sum(lons)/len(lons)
            coords_js = str([[p["lat"],p["lon"]] for p in route])
            accent = c["ACCENT"]
            card_bg = c["CARD"]
            border2 = c["BORDER2"]
            text3 = c["TEXT3"]
...
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<div style="border-radius:8px;overflow:hidden;border:1.5px solid {border2};background:{card_bg};">
  <div id="m{i}" style="width:100%;height:140px;"></div>
  <div style="padding:8px 10px;background:{card_bg};">
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <div style="font-family:monospace;font-size:11px;color:{accent};font-weight:700;">{dist} {unit}</div>
      {"<div style='font-family:monospace;font-size:9px;color:#FFB300;background:rgba(255,179,0,0.15);padding:2px 6px;border-radius:3px;'>PB</div>" if is_pb else ""}
    </div>
    <div style="font-family:monospace;font-size:10px;color:{text3};margin-top:2px;">{dur} · {pace}/{unit} · {date}</div>
  </div>
</div>
<script>
var m=L.map('m{i}',{{zoomControl:false,attributionControl:false,dragging:false,scrollWheelZoom:false}});
L.tileLayer('{osm}',{{maxZoom:19}}).addTo(m);
var coords={coords_js};
var poly=L.polyline(coords,{{color:'{accent}',weight:4,opacity:0.9}}).addTo(m);
m.fitBounds(poly.getBounds(),{{padding:[8,8]}});
setTimeout(function(){{m.invalidateSize();}},300);
</script>"""
            st.components.v1.html(map_html, height=220)


def page_run_debrief(user_id, profile, runs, unit, c):
    import requests, datetime
    st.markdown("<div class='page-title'>◈ RUN DEBRIEF</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>AI-POWERED POST-RUN ANALYSIS BY ARIA</div>", unsafe_allow_html=True)

    if not GROQ_API_KEY:
        st.error("⚠ GROQ_API_KEY required. Add to Streamlit Secrets.")
        return
    if not runs:
        st.info("Complete your first run to get an AI debrief!")
        return
    ...
    dist = format_distance(latest["distance_km"], unit)
    dur = format_time(latest["duration_seconds"])
    pace = format_pace(latest["duration_seconds"], latest["distance_km"], unit)
    date = latest.get("started_at","")[:10]
    is_pb = latest.get("is_pb", False)

    # Latest run card
    st.markdown(f"""
    <div style='background:{c["CARD"]};border:1.5px solid {c["ACCENT"]}60;border-radius:10px;padding:16px;margin-bottom:16px;position:relative;overflow:hidden;'>
        <div style='position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,{c["ACCENT"]},transparent);'></div>
        <div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;color:{c["TEXT3"]};letter-spacing:2px;margin-bottom:10px;'>◈ LATEST RUN · {date}</div>
        <div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;'>
            <div style='text-align:center;'><div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};'>DISTANCE</div><div style='font-family:Orbitron,sans-serif;font-size:1.1rem;font-weight:900;color:{c["ACCENT"]};'>{dist} {unit}</div></div>
            <div style='text-align:center;'><div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};'>DURATION</div><div style='font-family:Orbitron,sans-serif;font-size:1.1rem;font-weight:900;color:{c["TEXT"]};'>{dur}</div></div>
            <div style='text-align:center;'><div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:{c["TEXT3"]};'>AVG PACE</div><div style='font-family:Orbitron,sans-serif;font-size:1.1rem;font-weight:900;color:{c["TEXT"]};'>{pace}/{unit}</div></div>
        </div>
        {"<div style='margin-top:10px;font-family:Share Tech Mono,monospace;font-size:0.65rem;color:#FFB300;letter-spacing:2px;'>★ NEW PERSONAL BEST!</div>" if is_pb else ""}
    </div>""", unsafe_allow_html=True)

    # Auto-generate or use cached debrief
    debrief_key = f"debrief_{latest.get('id','')}"
    if debrief_key not in st.session_state:
        st.session_state[debrief_key] = None

    col1, col2 = st.columns(2)
    with col1:
        gen_btn = st.button("🤖 Generate ARIA Debrief", use_container_width=True)
    with col2:
        sel_run = st.selectbox("Analyze run", [f"Run {i+1} — {r.get('started_at','')[:10]} · {format_distance(r['distance_km'],unit)}{unit}" for i,r in enumerate(runs)], key="debrief_sel")
        sel_idx = int(sel_run.split()[1]) - 1 if sel_run else 0

    if gen_btn or st.session_state.get("auto_debrief"):
        st.session_state["auto_debrief"] = False
        sel_r = runs[sel_idx]
        sel_dist = format_distance(sel_r["distance_km"], unit)
        sel_dur = format_time(sel_r["duration_seconds"])
        sel_pace = format_pace(sel_r["duration_seconds"], sel_r["distance_km"], unit)
        prev_runs_str = "; ".join([f"{format_distance(r['distance_km'],unit)}{unit} @ {format_pace(r['duration_seconds'],r['distance_km'],unit)}/{unit}" for r in runs[1:4]]) if len(runs)>1 else "No previous runs"
        total_km = sum(r["distance_km"] for r in runs)
        prompt = f"""Analyze this specific run and give a detailed personal debrief:
Run: {sel_dist}{unit} in {sel_dur} at {sel_pace}/{unit} pace. Personal best: {sel_r.get('is_pb',False)}.
Runner: {rank['title']} LV.{rank['level']}, {xp} XP, {len(runs)} total runs, {total_km:.1f}km total.
Recent history: {prev_runs_str}.
Give: 1) What went well 2) What to improve 3) Comparison to recent runs 4) Specific next run recommendation.
Be specific, motivating, and concise. Under 250 words."""
        with st.spinner("🤖 ARIA is analyzing your run..."):
            try:
                resp = requests.post(
                    "https://api.groq.com/openai/v1/chat/completions",
                    headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"},
                    json={"model":"llama-3.1-8b-instant","max_tokens":400,"temperature":0.7,
                          "messages":[{"role":"system","content":"You are ARIA, an elite AI running coach. Be concise and motivating."},
                ...
                if resp.status_code == 200:
                    ...
                    if "choices" in data:
                        ...
                        result = f"API error: {data.get('error',{}).get('message','Unknown error')}"
                else:
                    result = f"HTTP {resp.status_code}: {resp.text[:200]}"
                st.session_state[debrief_key] = result
            except Exception as e:
                st.session_state[debrief_key] = f"Connection error: {str(e)}"

    if st.session_state.get(debrief_key):
        st.markdown(f"""
        <div style='background:{c["CARD"]};border:1.5px solid {c["ACCENT"]}40;border-radius:10px;padding:18px;margin-top:12px;position:relative;overflow:hidden;'>
            <div style='position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,{c["ACCENT"]},transparent);'></div>
            <div style='font-family:Orbitron,sans-serif;font-size:0.72rem;font-weight:700;color:{c["ACCENT"]};letter-spacing:2px;margin-bottom:12px;'>🤖 ARIA DEBRIEF</div>
        </div>""", unsafe_allow_html=True)
        st.markdown(st.session_state[debrief_key])


def page_share_card(user_id, runs, unit, c):
    import datetime
    st.markdown("<div class='page-title'>◈ SHARE RUN</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>GENERATE SHAREABLE RUN CARD</div>", unsafe_allow_html=True)

    if not runs:
        st.info("Complete a run first to generate a share card!")
        return

    sel = st.selectbox("Select run to share", [f"Run {i+1} — {r.get('started_at','')[:10]} · {format_distance(r['distance_km'],unit)}{unit}" for i,r in enumerate(runs)])
...
<div id="share-card" style="background:linear-gradient(135deg,#07090D,#0E1118);border:2px solid #FFB300;border-radius:16px;
    padding:24px;max-width:400px;margin:0 auto;position:relative;overflow:hidden;font-family:'Orbitron',sans-serif;">
  <div style="position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,#FFB300,#FF8C00,#FFD700);"></div>
  <div style="position:absolute;top:0;left:0;bottom:0;width:4px;background:linear-gradient(180deg,#FFB300,transparent);"></div>
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
    <div>
      <div style="font-size:9px;color:#FFB30080;letter-spacing:4px;margin-bottom:4px;">[ SYS:ACTIVE ]</div>
      <div style="font-size:22px;font-weight:900;letter-spacing:2px;"><span style="color:#FFB300;">RUN</span><span style="color:#F0E6C8;">QUEST</span></div>
    </div>
    <div style="text-align:right;">
      <div style="font-size:9px;color:#5A4A2A;letter-spacing:2px;font-family:'Share Tech Mono',monospace;">{rank_sym} {rank["title"].upper()}</div>
      <div style="font-size:9px;color:#5A4A2A;font-family:'Share Tech Mono',monospace;">LV.{rank["level"]}</div>
    </div>
  </div>
  <div style="margin-bottom:20px;">
    <div style="font-size:9px;color:#5A4A2A;letter-spacing:3px;font-family:'Share Tech Mono',monospace;margin-bottom:4px;">OPERATIVE</div>
    <div style="font-size:14px;color:#FFB300;letter-spacing:2px;">{username}</div>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px;">
    <div style="text-align:center;background:rgba(255,179,0,0.08);border:1px solid #FFB30040;border-radius:8px;padding:12px 6px;">
      <div style="font-size:9px;color:#5A4A2A;font-family:'Share Tech Mono',monospace;letter-spacing:1px;margin-bottom:6px;">KM</div>
      <div style="font-size:20px;font-weight:900;color:#FFB300;">{dist}</div>
    </div>
    <div style="text-align:center;background:rgba(255,179,0,0.08);border:1px solid #FFB30040;border-radius:8px;padding:12px 6px;">
      <div style="font-size:9px;color:#5A4A2A;font-family:'Share Tech Mono',monospace;letter-spacing:1px;margin-bottom:6px;">TIME</div>
      <div style="font-size:16px;font-weight:900;color:#F0E6C8;">{dur}</div>
    </div>
    <div style="text-align:center;background:rgba(255,179,0,0.08);border:1px solid #FFB30040;border-radius:8px;padding:12px 6px;">
      <div style="font-size:9px;color:#5A4A2A;font-family:'Share Tech Mono',monospace;letter-spacing:1px;margin-bottom:6px;">PACE</div>
      <div style="font-size:14px;font-weight:900;color:#F0E6C8;">{pace}</div>
    </div>
  </div>
  {"<div style='background:rgba(255,179,0,0.15);border:1px solid #FFB300;border-radius:6px;padding:8px;text-align:center;font-size:11px;color:#FFB300;letter-spacing:3px;margin-bottom:16px;'>★ NEW PERSONAL BEST ★</div>" if is_pb else ""}
  <div style="display:flex;justify-content:space-between;align-items:center;">
    <div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:#3A3020;letter-spacing:2px;">{date}</div>
    <div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:#FFB300;letter-spacing:2px;">+{xp} XP</div>
    <div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:#3A3020;letter-spacing:2px;">nyk.streamlit.app</div>
  </div>
</div>
<div style="text-align:center;margin-top:12px;font-family:monospace;font-size:11px;color:#888;">Screenshot this card to share your run!</div>"""

    st.components.v1.html(card_html, height=420)
...
def main():
    ...
    elif page == "Challenges":    page_challenges(user_id, profile, runs, unit, c)
    elif page == "Leaderboard":   page_leaderboard(user_id, profile, runs, unit, c)
    elif page == "Activity Map":  page_heatmap(user_id, runs, c)
    elif page == "Route Gallery": page_route_gallery(user_id, runs, unit, c)
    elif page == "Run Debrief":   page_run_debrief(user_id, profile, runs, unit, c)
    elif page == "Share Run":     page_share_card(user_id, runs, unit, c)
