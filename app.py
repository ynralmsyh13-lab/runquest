...
def page_quests(user_id, profile, runs, c):
    ...
    if st.session_state.selected_rank is not None:
        ...
        status = "✅ UNLOCKED" if unlocked_sr else f"🔒 LOCKED · Need {sr['min_xp']} XP"
        is_cur_sr = sr["level"] == rank["level"]
        st.markdown(f"""
        <div style='background:{c["CARD"]}; border:1.5px solid {c["ACCENT"] if is_cur_sr else c["BORDER2"]};
            border-radius:8px; padding:12px 16px; margin-top:8px; display:flex;
            ...
            <div style='font-size:1.8rem;'>{sr["icon"]}</div>
            <div>
                <div style='font-family:Orbitron,sans-serif; font-size:1rem; font-weight:900;
                    color:{c["ACCENT"]}; letter-spacing:2px;'>{sr["title"].upper()}</div>
                <div style='font-family:Share Tech Mono,monospace; font-size:0.65rem;
                    color:{c["TEXT3"]}; letter-spacing:1px; margin-top:2px;'>
                    LEVEL {sr["level"]} · {sr["min_xp"]:,} XP REQUIRED · {status}
                    {"· ◈ CURRENT RANK" if is_cur_sr else ""}
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
