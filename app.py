...
def page_quests(user_id, profile, runs, c):
    ...
    # Short 3-letter codes for each rank
    RANK_CODES = ["WDR","JGR","PCR","STR","RCR","SPR","CHP","LGD","MYT","TRN"]

    rank_html = f"<div style='display:flex; gap:5px; overflow-x:auto; padding-bottom:6px;'>"
    for i, r in enumerate(RANKS):
        is_cur = r["level"] == rank["level"]
        unlocked = xp >= r["min_xp"]
        code = RANK_CODES[i] if i < len(RANK_CODES) else r["title"][:3].upper()
        border_col = c["ACCENT"] if is_cur else (c["BORDER2"] if unlocked else c["BORDER"])
        bg = "rgba(255,179,0,0.12)" if is_cur else "transparent"
        text_col = c["ACCENT"] if is_cur else (c["TEXT3"] if unlocked else c["TEXT3"])
        opacity = "1" if unlocked else "0.25"
        shadow = f"box-shadow:0 0 10px rgba(255,179,0,0.25);" if is_cur else ""
        rank_html += f"""
        <div style='flex-shrink:0; width:44px; text-align:center; padding:8px 2px 6px;
            border-radius:6px; border:2px solid {border_col};
            background:{bg}; opacity:{opacity}; {shadow}'>
            <div style='font-size:1.1rem; line-height:1.2;'>{r["icon"]}</div>
            <div style='font-family:Orbitron,sans-serif; font-size:0.5rem; font-weight:700;
                color:{text_col}; margin-top:3px; letter-spacing:0.5px;'>{code}</div>
            {"<div style='font-size:0.45rem; color:" + c["ACCENT"] + "; font-family:Share Tech Mono,monospace; margin-top:1px; letter-spacing:1px;'>▲NOW</div>" if is_cur else ""}
        </div>"""
    rank_html += "</div>"
