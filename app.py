...
function gst(){{
    ...
    if(tD > 0.0005 && tS > 0){{
        setStatus('<span style="color:{c["ACCENT"]};font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ SAVING RUN... ]</span>');
        // Navigate parent window to trigger Streamlit with query params
        var url = window.parent.location.href.split('?')[0];
        url += '?rq_dist=' + tD.toFixed(4) + '&rq_dur=' + tS + '&rq_unit=' + unit;
...
def page_gps_tracker(user_id, profile, runs, unit, c):
    ...
    # Auto-save from HALT button via query params
    params = st.query_params
    if "rq_dist" in params and "rq_dur" in params:
        try:
            dist_raw = float(params["rq_dist"])
            dur_raw  = int(params["rq_dur"])
            u_raw    = params.get("rq_unit", unit)
            dist_km  = dist_raw if u_raw == "km" else dist_raw / 0.621371
            st.query_params.clear()
            if dist_km > 0.0005 and dur_raw > 0:
    ...
    st.caption("💡 Go outside · Wait for GPS accuracy <15m · Press HALT to auto-save your run")
