...
function gst(){{
    ...
    setStatus('<span style="color:{c["ACCENT"]};font-family:Share Tech Mono,monospace;font-size:0.7rem;letter-spacing:1px;">[ RUN COMPLETE — ENTER DATA BELOW TO SAVE ]</span>');
...
def page_gps_tracker(user_id, profile, runs, unit, c):
    ...
    # GPS live tracker display
    gps_component(unit_sel, c)

    st.markdown("<hr class='divider'>", unsafe_allow_html=True)
    st.markdown(f"""
    <div style='background:{c["CARD"]}; border:1.5px solid {c["BORDER2"]};
        ...
        <div style='font-family:Orbitron,sans-serif; font-size:0.8rem; font-weight:700;
            color:{c["ACCENT"]}; letter-spacing:2px; margin-bottom:6px;'>◈ SAVE RUN</div>
        <div style='font-family:Exo 2,sans-serif; font-size:0.85rem; color:{c["TEXT2"]}; margin-bottom:14px;'>
            After pressing <b>HALT</b>, enter your distance and time from the display above, then save.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.form("gps_save_form", clear_on_submit=True):
        ...
        with col1:
            dist_val = st.number_input("Distance", min_value=0.001,
                                       max_value=200.0, step=0.01,
                                       value=0.01, format="%.3f")
        with col2:
            unit_save = st.selectbox("Unit", ["km", "mi"],
                                     index=0 if unit_sel == "km" else 1)
        col3, col4, col5 = st.columns(3)
        with col3: hrs = st.number_input("HH", min_value=0, max_value=24, value=0)
        with col4: mins = st.number_input("MM", min_value=0, max_value=59, value=0)
        with col5: secs = st.number_input("SS", min_value=0, max_value=59, value=0)

        if st.form_submit_button("◈ SAVE RUN TO HISTORY", use_container_width=True):
            dist_km = dist_val if unit_save == "km" else dist_val / 0.621371
            ...
            if total_secs == 0:
                ...
                total_xp, is_pb, fresh = process_and_save_run(
                    user_id, profile, runs, dist_km, total_secs, unit_save, []
                )
                st.session_state["last_run_saved"] = {
                    "dist": dist_km, "dur": total_secs, "xp": total_xp,
                    "is_pb": is_pb, "fresh": fresh, "unit": unit_save
                }
                st.rerun()

    st.caption("💡 Go outside · Wait for GPS accuracy <15m · Keep phone in hand or armband")
