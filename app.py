...
def render_bottom_nav(page, c):
    # Only 6 items in bottom nav (no settings)
    bottom_pages = NAV_PAGES[:6]
    st.markdown("<div class='bottom-nav-container'>", unsafe_allow_html=True)
    cols = st.columns(len(bottom_pages))
    for i, (icon, name) in enumerate(bottom_pages):
        is_active = page == name
        with cols[i]:
            css_class = "nav-active" if is_active else ""
            st.markdown(f"<div class='{css_class}'>", unsafe_allow_html=True)
            if st.button(f"{icon}\n{name.split()[0][:5].upper()}", key=f"bnav_{name}"):
...
def render_sidebar(profile, page, c):
    with st.sidebar:
        ...
        # Logo
        st.markdown(f"""
        <div style='padding:20px 16px 14px; border-bottom:1px solid {c["BORDER2"]}50;'>
            ...
            <div class='rq-logo'>RUN<span style='-webkit-text-fill-color:{c["TEXT"]}; background:none;'>QUEST</span></div>
            <div style='font-family:Share Tech Mono,monospace; font-size:0.55rem; color:{c["TEXT3"]}; letter-spacing:5px; margin-top:3px;'>PERFORMANCE OS v2.0</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Account + signout
        st.markdown(f"""
        <div style='padding:10px 16px; border-top:1px solid {c["BORDER2"]}50; margin-top:8px;'>
            ...
            <div style='font-family:Exo 2,sans-serif; font-size:0.85rem; font-weight:600; color:{c["TEXT2"]}; word-break:break-all; margin-bottom:10px;'>{st.session_state.user.email}</div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("⏻  TERMINATE SESSION", key="signout"):
            sb.auth.sign_out()
