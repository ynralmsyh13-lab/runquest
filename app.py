...
def page_quests(user_id, profile, runs, c):
    ...
    def quest_svg(quest_id, active=True):
        svg = QUEST_SVGS.get(quest_id, QUEST_SVGS["first_run"])
        clr = c["ACCENT"] if active else "#00C853"
        fil = "rgba(255,179,0,0.15)" if active else "rgba(0,200,83,0.15)"
        return svg.replace("CLRX", clr).replace("FILX", fil)
...
def page_settings(user_id, profile, c):
    ...
    joined = st.session_state.user.created_at[:10] if hasattr(st.session_state.user, 'created_at') else "—"
