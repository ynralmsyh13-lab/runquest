...
def render_route_map(route, c, height=300):
    ...
    <div id="rmap" style="width:100%;height:{height}px;border-radius:8px;border:1.5px solid {border};overflow:hidden;"></div>
    <script>
    var map = L.map('rmap', {{zoomControl:true, attributionControl:true}}).setView([{clat},{clon}], 16);
    L.tileLayer('https://{{s}}.basemaps.cartocdn.com/light_all/{{z}}/{{x}}/{{y}}{{r}}.png', {{
        attribution: '© OpenStreetMap © CARTO', subdomains:'abcd', maxZoom:20
    }}).addTo(map);
    var coords = {coords_js};
    var poly = L.polyline(coords, {{color:'{accent}', weight:5, opacity:0.95, lineCap:'round', lineJoin:'round'}}).addTo(map);
    var si = L.divIcon({{html:'<div style="width:14px;height:14px;border-radius:50%;background:#00C853;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);"></div>', className:'', iconSize:[14,14], iconAnchor:[7,7]}});
    var ei = L.divIcon({{html:'<div style="width:14px;height:14px;border-radius:50%;background:#FF4444;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);"></div>', className:'', iconSize:[14,14], iconAnchor:[7,7]}});
    L.marker([{start_p["lat"]},{start_p["lon"]}], {{icon:si}}).addTo(map).bindPopup('<b>🟢 START</b>');
    L.marker([{end_p["lat"]},{end_p["lon"]}], {{icon:ei}}).addTo(map).bindPopup('<b>🔴 FINISH</b>');
...
<style>@keyframes pg {{0%,100%{{opacity:1;}} 50%{{opacity:0.3;}}}}</style>
<div style="font-family:'Exo 2',sans-serif;">
  ...
  <div style="display:flex;gap:8px;margin-bottom:12px;">
    ...
    <button id="b-pause" onclick="gp()" style="display:none;flex:1;padding:13px 6px;border-radius:6px;border:2px solid #FF8C00;cursor:pointer;background:{card};color:#FF8C00;font-family:'Orbitron',sans-serif;font-size:0.7rem;font-weight:800;letter-spacing:2px;">⏸ PAUSE</button>
    <button id="b-resume" onclick="gr()" style="display:none;flex:1;padding:13px 6px;border-radius:6px;border:2px solid {accent};cursor:pointer;background:rgba(255,179,0,0.12);color:{accent};font-family:'Orbitron',sans-serif;font-size:0.7rem;font-weight:800;letter-spacing:2px;">▶ RESUME</button>
    <button id="b-stop" onclick="gst()" style="display:none;flex:1;padding:13px 6px;border-radius:6px;border:2px solid #FF5555;cursor:pointer;background:{card};color:#FF5555;font-family:'Orbitron',sans-serif;font-size:0.7rem;font-weight:800;letter-spacing:2px;">■ HALT</button>
  </div>
  ...
  <div id="sum" style="display:none;background:{card};border:1.5px solid {border2};border-radius:8px;padding:16px;position:relative;overflow:hidden;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
      <div style="text-align:center;"><div style="font-size:0.7rem;color:{text3};margin-bottom:4px;">DISTANCE</div><div id="sd" style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{accent};"></div></div>
      <div style="text-align:center;"><div style="font-size:0.7rem;color:{text3};margin-bottom:4px;">DURATION</div><div id="st2" style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{text_c};"></div></div>
      <div style="text-align:center;"><div style="font-size:0.7rem;color:{text3};margin-bottom:4px;">AVG PACE</div><div id="sp" style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{text_c};"></div></div>
      <div style="text-align:center;"><div style="font-size:0.7rem;color:{text3};margin-bottom:4px;">STATUS</div><div style="font-family:'Orbitron',sans-serif;font-size:1.2rem;font-weight:800;color:{accent};">DONE ✓</div></div>
    </div>
    <div id="summap" style="width:100%;height:200px;border-radius:6px;overflow:hidden;border:1px solid {border2};margin-bottom:14px;"></div>
...
var route=[],liveMap=null,liveMarker=null,livePoly=null,sumMap=null;
...
function initLiveMap(lat,lon){{
    ...
    if(liveMap){{liveMap.remove();liveMap=null;livePoly=null;liveMarker=null;}}
    liveMap=L.map('livemap',{{zoomControl:true}}).setView([lat,lon],17);
    L.tileLayer('https://{{s}}.basemaps.cartocdn.com/light_all/{{z}}/{{x}}/{{y}}{{r}}.png',{{attribution:'© CARTO',subdomains:'abcd',maxZoom:20}}).addTo(liveMap);
    var si=L.divIcon({{html:'<div style="width:12px;height:12px;border-radius:50%;background:#00C853;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);"></div>',className:'',iconSize:[12,12],iconAnchor:[6,6]}});
    L.marker([lat,lon],{{icon:si}}).addTo(liveMap).bindPopup('🟢 START');
    var li=L.divIcon({{html:'<div style="width:16px;height:16px;border-radius:50%;background:{accent};border:3px solid white;box-shadow:0 0 12px {accent};animation:pg 1.5s infinite;"></div>',className:'',iconSize:[16,16],iconAnchor:[8,8]}});
    liveMarker=L.marker([lat,lon],{{icon:li,zIndexOffset:1000}}).addTo(liveMap);
    livePoly=L.polyline([[lat,lon]],{{color:'{accent}',weight:5,opacity:0.95,lineCap:'round',lineJoin:'round'}}).addTo(liveMap);
}}
function updateLiveMap(lat,lon){{
    if(!liveMap)return;
    if(liveMarker)liveMarker.setLatLng([lat,lon]);
    if(livePoly)livePoly.addLatLng([lat,lon]);
...
function initSumMap(){{
    ...
    setTimeout(function(){{
        ...
        L.tileLayer('https://{{s}}.basemaps.cartocdn.com/light_all/{{z}}/{{x}}/{{y}}{{r}}.png',{{subdomains:'abcd',maxZoom:20}}).addTo(sumMap);
        var coords=route.map(p=>[p.lat,p.lon]);
        var poly=L.polyline(coords,{{color:'{accent}',weight:5,opacity:0.95,lineCap:'round',lineJoin:'round'}}).addTo(sumMap);
        var si=L.divIcon({{html:'<div style="width:12px;height:12px;border-radius:50%;background:#00C853;border:3px solid white;"></div>',className:'',iconSize:[12,12],iconAnchor:[6,6]}});
        var ei=L.divIcon({{html:'<div style="width:12px;height:12px;border-radius:50%;background:#FF4444;border:3px solid white;"></div>',className:'',iconSize:[12,12],iconAnchor:[6,6]}});
        L.marker(coords[0],{{icon:si}}).addTo(sumMap).bindPopup('🟢 START');
        L.marker(coords[coords.length-1],{{icon:ei}}).addTo(sumMap).bindPopup('🔴 FINISH');
...
function gs(){{
    ...
    wId=navigator.geolocation.watchPosition(
        ...
        function(e){{setStatus('<span style="color:#FF5555;font-size:0.7rem;">⚠ GPS: '+e.message+'</span>');}},
        {{enableHighAccuracy:true,maximumAge:0,timeout:10000}}
    );
}}
function gp(){{paused=true;document.getElementById('b-pause').style.display='none';document.getElementById('b-resume').style.display='flex';setStatus('<span style="color:#FF8C00;font-family:Share Tech Mono,monospace;font-size:0.75rem;">⏸ PAUSED</span>');}}
function gr(){{paused=false;lP=null;document.getElementById('b-resume').style.display='none';document.getElementById('b-pause').style.display='flex';setStatus('<span style="color:{accent};font-family:Share Tech Mono,monospace;font-size:0.75rem;">● TRACKING ACTIVE</span>');}}
function gst(){{
