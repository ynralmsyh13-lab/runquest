...
def page_quests(user_id, profile, runs, c):
    ...
    for i, r in enumerate(RANKS):
        ...
        with cols[i]:
            if is_cur:
                st.markdown(f"<div style='text-align:center;border:2px solid {c['ACCENT']};border-radius:6px;padding:4px 1px;background:rgba(255,179,0,0.12);'><div style='font-size:18px;'>{r['icon']}</div><div style='font-size:8px;font-family:monospace;color:{c['ACCENT']};'>▲</div></div>", unsafe_allow_html=True)
            elif unlocked:
                st.markdown(f"<div style='text-align:center;border:1px solid {c['BORDER2']};border-radius:6px;padding:4px 1px;opacity:0.8;'><div style='font-size:18px;'>{r['icon']}</div></div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='text-align:center;border:1px solid {c['BORDER2']}40;border-radius:6px;padding:4px 1px;opacity:0.25;'><div style='font-size:18px;'>{r['icon']}</div></div>", unsafe_allow_html=True)
